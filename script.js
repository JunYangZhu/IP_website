$(document).ready(function () {
      ...
      $.ajax(settings).done(function (response) {
        ...
        for (var i = 0; i < response.length && i < limit; i++) {
          ...
          */
          ...
          <td>${response[i].email}</td>
          <td>${response[i].pw}</td>
          <td><a href='#' class='delete' data-id='${response[i]._id}'>Del</a></td><td><a href='#update-account-container' class='update' data-id='${response[i]._id}' data-pw='${response[i].pw}' data-name='${response[i].name}' data-email='${response[i].email}'>Update</a></td></tr>`;
    ...
    function updateForm(id, accountName, accountEmail, accountPassword) {
      ...
      var jsondata = { "name": accountName, "email": accountEmail, "pw": accountPassword };
      var settings = {
        "async": true,
        "crossDomain": true,
